...
aapl <- na.omit(aapl) %>% select(-c(Close, compound))
...
recipe <- recipe(nextClose ~ Open + High + Low + Adjusted + Volume + neg + neu + pos, data = aapl)
...
# calculate the RMSE
RMSE[i] <- sqrt(mean((pred - aapl_test$nextClose)^2, na.rm = TRUE))
}
...
```{r}
# epoch = 20
# (1.19049 + 0.8962939 + 1.052433 + 1.095134 + 1.017034)/5
```
