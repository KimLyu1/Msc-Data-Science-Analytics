...
train_X <- reshapeForLSTMLoop(train_data, numTimeSteps = 2, columnsToExclude = "nextClose")
train_Y <- as.matrix(tail(train_data$nextClose, -2))
test_X <- reshapeForLSTMLoop(test_data, numTimeSteps = 2, columnsToExclude = "nextClose")
test_Y <- as.matrix(tail(test_data$nextClose, -2))
```
...
# inverse normalize
pred <- c(rep(NA, 2), pred * ranges[2,4])
...
```{r}
# epoch = 20
# (3.457972 + 3.363598 + 3.679038 + 3.032637 + 3.490657)/5

# re-record
# (3.212639 + 2.772958 + 2.858017 + 2.991184 + 3.066037)/5
```
